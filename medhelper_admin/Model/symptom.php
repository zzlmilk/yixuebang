...
class symptomModel extends baseModel{
	...
	public function __construct($id = 0) {
        ...
        if (!empty($id)) {
            ...
            $this->article_info = M('medhelper_symptom')->where("id = " . $id)->find();
            ...
            $this->article_info['content'] = M('medhelper_article')->where('article_type = 3 and table_id = '.$id)->find();
        }
    }
    ...
    public function createAndUpdateData($value){
    	...
    	switch($value){
    		...
    		case 0:
    			...
    			$this->id = M('medhelper_symptom')->add($this->field);

    			break;

    		case 1:

    			M('medhelper_symptom')->where("id = ".$this->id)->save($this->field);
        ...
        if(!empty($this->id)){
            ...
            $article_info = M('medhelper_article')->where("article_type = 3 and table_id = ".$this->id)->find();

            if($article_info != NULL){

                M('medhelper_article')->where("article_type = 3 and table_id = ".$this->id)->save($create);
    ...
    public function getList() {
        ...
        $this->fenye('symptom', 'symptom_list');
    }

    public function getTotal() {

        $this->info = M('medhelper_symptom')->field('id')->select();
    ...
    public function getInfoByPage() {
        ...
        $info = M('medhelper_symptom')->field('id,type_id_1,type_id_2,type_id_3,create_time,update_time')->limit($this->limit)->offset($this->offset)->select();
