...
class operationModel extends baseModel{
    ...
    public function __construct($id = 0) {
        ...
        if (!empty($id)) {
            ...
            $this->article_info = M('medhelper_operation')->where("id = " . $id)->find();
            ...
            $this->article_info['content'] = M('medhelper_article')->where('article_type = '.$this->type.' and table_id = ' . $id)->find();
        }
    }
    ...
    public function createAndUpdateData($value) {
        ...
        switch ($value) {
            ...
            case 0:
                ...
                $this->id = M('medhelper_operation')->add($this->field);

                break;

            case 1:

                M('medhelper_operation')->where("id = " . $this->id)->save($this->field);
        ...
        if (!empty($this->id)) {
            ...
            $article_info = M('medhelper_article')->where("article_type = ".$this->type." and table_id = " . $this->id)->find();

            if ($article_info != NULL) {

                M('medhelper_article')->where("article_type = ".$this->type." and table_id = " . $this->id)->save($create);
            } else {
    ...
    public function getList() {
        ...
        $this->fenye('operation', 'operation_list');
    }

    public function getTotal() {

        $this->info = M('medhelper_operation')->field('id')->select();
    ...
    public function getInfoByPage() {
        ...
        $info = M('medhelper_operation')->limit($this->limit)->offset($this->offset)->select();
        ...
        if ($info_count > 0) {
            ...
            foreach ($info as $key => $value) {
                ...
                $info[$key]['content'] = M('medhelper_article')->where(' article_type = '.$this->type.' and table_id = ' . $value['id'])->find();
            }
