...
class medicalModel extends baseModel{
	...
	public function __construct($id = 0) {
        ...
        if (!empty($id)) {
            ...
            $this->article_info = M('medhelper_medical')->where("id = " . $id)->find();
            ...
            $this->article_info['content'] = M('medhelper_article')->where('article_type = 9 and table_id = '.$id)->find();
        }
    }
    ...
    public function createAndUpdateData($value){
    	...
    	switch($value){
    		...
    		case 0:
    			...
    			$this->id = M('medhelper_medical')->add($this->field);

    			break;

    		case 1:

    			M('medhelper_medical')->where("id = ".$this->id)->save($this->field);
        ...
        if(!empty($this->id)){
            ...
            $article_info = M('medhelper_article')->where("article_type = 9 and table_id = ".$this->id)->find();

            if($article_info != NULL){

                M('medhelper_article')->where("article_type = 9 and table_id = ".$this->id)->save($create);
    ...
    public function setArrayValue(){
    	...
    	$array = array('type_id_1','type_id_2');
    ...
    public function getTotal() {
        ...
        $this->info = M('medhelper_medical')->field('id')->select();
    ...
    public function getInfoByPage() {
        ...
        $info = M('medhelper_medical')->limit($this->limit)->offset($this->offset)->select();
        ...
        if ($info_count > 0) {
            ...
            foreach ($info as $key => $value) {
                ...
                $info[$key]['content'] = M('medhelper_article')->where(' article_type = 9 and table_id = '.$value['id'])->find();
            }
